...
class TestNdefCodec(unittest.TestCase):
    ...
    def test_oversized_uri_raises_issueerror(self):
        # payload = prefix(1) + rest; rest of 252 bytes → payload 253 > 250
        long_url = "x" * 252  # no known prefix → prefix 0x00, payload = 1 + 252 = 253 > 250
        with self.assertRaises(il.IssueError):
            il.encode_ndef_uri(long_url)
        # a URL right at the limit must still encode without error
        ok_url = "x" * 249  # payload = 1 + 249 = 250, record = 254 (fits)
